/* ------------------------------------------------------------------------ *\
	...
	TITLE   :  2-D Poisson's equation solver (OpenACC version)
	DATE    :  2018-12-08
\* ------------------------------------------------------------------------ */
	...
	double * const T2)
{
	...
	#pragma acc parallel loop present(S[0:l_from_nXY(nX,nY)],T[0:l_from_nXY(nX,nY)],T2[0:l_from_nXY(nX,nY)])
	...
	char * const argv[])
{
	...
	#pragma acc data copyout(T[0:l_from_nXY(nX,nY)]) create(S[0:l_from_nXY(nX,nY)],T2[0:l_from_nXY(nX,nY)])
	{
		project_init_1(
			aX,
			aY,
